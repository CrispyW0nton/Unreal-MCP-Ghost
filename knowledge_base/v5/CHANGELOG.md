...
- Added a dedicated Texture/Paint **Paint Stroke** section in the Unreal Generate Asset panel for part name plus generated texture image bucket/key, URL, or file token, and threaded those fields into the inserted `gen_record_texture_paint_stroke` workflow.
...
- Added no-spend live Tripo wallet checking through `gen_tripo_get_wallet_balance`, plus local task-credit reconciliation from Tripo `consumed_credit` so the Generative Credits display can move from reserved estimates to actual provider consumption after status polling.
- Added **Check Wallet** buttons to the Unreal Generate Asset and Generate Settings credits cards so users can insert the no-spend live wallet-balance check without knowing the MCP tool name.
