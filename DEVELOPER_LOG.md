...
| BUG-040 | `set_component_property` / `SetObjectProperty` | Setting `SkeletalMesh`, `Material`, `Texture`, or any `UObject*` asset reference via `set_component_property` silently fails — returns `"Unsupported property type: FObjectProperty"` | ✅ Fixed (2026-04-18) | Added `FObjectProperty` + `FSoftObjectProperty` handler to `SetObjectProperty` in `UnrealMCPCommonUtils.cpp`; uses `FindObject` then `StaticLoadObject` to resolve asset path string |
| BUG-041 | No tool for SkeletalMeshComponent mesh/material assignment | Armor pieces are **untextured** — no dedicated C++ command or Python tool existed to assign a `USkeletalMesh` asset + per-slot `UMaterialInterface` overrides to a `SkeletalMeshComponent` in the Blueprint SCS | ✅ Fixed (2026-04-18) | New C++ command `set_skeletal_mesh_properties` + Python tool `set_skeletal_mesh_properties`. Supports `skeletal_mesh`, `material` (slot-0 shorthand), and `materials` (per-slot array). BUG-A: uses `SetSkeletalMeshAsset()` (not `SetSkeletalMesh()`) — safe on CDO templates. BUG-B: uses `SkelComp->SetMaterial(slot, mat)` (not direct `OverrideMaterials[]`) — handles array resize correctly |
| BUG-042 | No tool for SCS socket attachment | Armor pieces **not snapping to the right places** — no C++ command or Python tool existed to reparent an SCS node to a named bone socket (e.g. `"hand_r"`, `"spine_01"`) on a parent `SkeletalMeshComponent` | ✅ Fixed (2026-04-18) | New C++ command `set_component_parent_socket` + Python tool `set_component_parent_socket`. Sets `USCS_Node::AttachToName` (the UE5 SCS attachment socket) and optionally reparents the node to a new parent SCS node. BUG-C: uses `SCS->FindParentNode()` (no `USCS_Node::GetParent()` exists). BUG-D: uses `OldParent->RemoveChildNode()` + `NewParentNode->AddChildNode()` (not `SCS->RemoveNode()` which destroys subtrees) |

| BUG-043 | Missing `ctx: Context` param in 4 MCP tools | `set_blueprint_ai_controller` (blueprint_tools.py), `add_input_mapping` (project_tools.py), `list_knowledge_base_topics`, `get_knowledge_base`, `search_knowledge_base` (knowledge_tools.py) all registered with `@mcp.tool()` but without `ctx: Context` first param — inconsistent with rest of codebase and prevents context access | ✅ Fixed (2026-04-18) | Added `ctx: Context` to `set_blueprint_ai_controller`. Confirmed `add_input_mapping` and knowledge tools are OK in MCP 1.27 (ctx is optional) — left as-is since they don't need context features |
