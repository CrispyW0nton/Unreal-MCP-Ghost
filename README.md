...
GenSpark will connect via MCP SSE and automatically discover all 311 tools. See [Section 14](#14-ai-agent-onboarding-prompt) for the onboarding prompt to paste into your first GenSpark session.
...
The MCP server exposes **311 Python tools** across 18 tool modules. All tools are called by the AI agent via its MCP connection — local agents call them directly, remote agents call them via SSE. The 311 Python tools cover all 20 chapters of "Blueprints Visual Scripting for Unreal Engine 5" and internally translate to 119 C++ plugin commands.
...
Every AI agent — local or remote — uses the same 311 MCP tools. There is **no separate "cloud mode"** or special CLI interface; the MCP connection (stdio for local, SSE for remote) is the one and only interface.
...
1. CALL MCP TOOLS ONLY. You interact with UE5 exclusively through MCP tool calls.
   Do not attempt to run shell commands. All 311 tools are available directly.
