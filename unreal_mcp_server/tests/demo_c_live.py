...
passed = 0
failed = 0
fail_fast = True  # stop on first failure per spec
...
def step(n: int, name: str, fn, *args, **kwargs):
    ...
    line = f"[{status}] step_{n:02d} ({name}) duration_ms={elapsed_ms} token_estimate={tokens} notes=\"{notes}\""
    print(line)
    ...
    if ok:
        ...
        failed += 1
        if fail_fast:
            print(f"\n[ABORT] Step {n} failed — stopping. Run with fail_fast=False to continue.")
            _print_summary()
            sys.exit(1)
...
def _print_summary():
    ...
    print("\n" + "="*70)
    print(f"Demo C — Phase 3 Project Intelligence")
...
def step1_ping() -> Tuple[bool, str, int]:
    """Step 1 — Ping UE5."""
    r = send_raw("ping", {})
    ok = (r.get("success") is not False and
          (r.get("result", {}).get("message") == "pong" or "pong" in str(r).lower()))
    return ok, f"response={json.dumps(r)[:200]}", 0


def step2_list_subsystems() -> Tuple[bool, str, int]:
    """Step 2 — project_list_subsystems(editor)."""
    code = """
import unreal
...
"""
    ...
    out = (r.get("result") or r.get("outputs") or {})
    ...
    return ok, f"editor subsystems found={len(editor_classes)}, UEditorAssetSubsystem={'yes' if ok else 'MISSING'}", tokens
...
if bp is None:
    ...
    bp = next((unreal.load_asset(str(h.object_path)) for h in hits if h.asset_name == 'BP_HealthSystem'), None)
if bp:
...
# Re-use result from step 4
_step4_result: Dict[str, Any] = {}


def step4_wrapper() -> Tuple[bool, str, int]:
    global _step4_result
    ok, notes, tokens = step4_graph_summary_variables()
    return ok, notes, tokens
...
"""
    ...
    ok  = "TakeDamage" in fg
    tokens = len(json.dumps(out)) // 4
    return ok, f"function_graphs={sorted(fg)}, TakeDamage={'yes' if ok else 'MISSING'}", tokens
...
def step6_graph_detail_tokens() -> Tuple[bool, str, int]:
    ...
    ok = token_est < 1800
    return ok, f"TakeDamage compact nodes={len(nodes_compact)}, token_estimate={token_est} (<1800={'yes' if ok else 'OVER'})", token_est
...
"""
    ...
    tokens = len(json.dumps(out)) // 4
    return ok, (f"referencers={len(out.get('referencers',[]))} deps={len(out.get('dependencies',[])) } "
...
"""
    ...
    tokens = len(json.dumps(out)) // 4
    return ok, f"blueprints_with_actor_parent={sorted(names)[:6]}, BP_DemoA={'found' if ok else 'MISSING'}", tokens
...
"""
    ...
    tokens = len(json.dumps(out)) // 4
    return ok, (f"chain_nodes={len(out.get('nodes',[]))} depth_reached={out.get('depth_reached',0)} "
                f"depth_reached_key={'yes' if ok else 'MISSING'}"), tokens
...
def step10_cpp_set_path() -> Tuple[bool, str, int]:
    ...
    if resolved and os.path.isdir(resolved):
        ...
        ok = len(files) > 0
        return ok, f"path={resolved}, files_indexed={len(files)}", 0
...
def step11_cpp_analyze_class() -> Tuple[bool, str, int]:
    ...
    # Try both the plugin class and a fallback
    for cls_name in ["UUnrealMCPBridge", "FUnrealMCPEditorCommands", "FUnrealMCPBlueprintCommands"]:
        ...
        if result:
            ...
            return ok, (f"class={cls_name} parent={result.get('parent_class','?')} "
                        f"methods={len(methods)} file={os.path.basename(result.get('header_file','?'))}"), 0

    # No UCLASS found (plain C++ class is acceptable)
    # Check if any file mentions the class at least
    any_match = any(
        "UUnrealMCPBridge" in open(f, encoding="utf-8", errors="replace").read()
        for f in m._INDEXED_FILES[:20] if f.endswith(".h")
    )
    if any_match:
        return True, "UUnrealMCPBridge found in headers (no UCLASS macro — plain class)", 0
...
def step12_cpp_find_references() -> Tuple[bool, str, int]:
    ...
    if ok:
        hit = result["hits"][0]
        return ok, (f"total={result['total']}, first_hit=file:{os.path.basename(hit['file'])} "
                    f"line:{hit['line']} snippet:\"{hit['snippet'][:80]}\""), 0
...
try:
    ...
    _result['provider']  = str(sc.get_provider_name()) if sc.get_provider_name() else 'None'
...
"""
    r   = exec_python_remote(code)
    out = (r.get("result") or r.get("outputs") or {})
    # Success = we got a dict with 'provider' and 'available' keys
    ok  = "provider" in out and "available" in out
    if not ok and not r.get("success", True):
        # UE not connected — still acceptable, we just stub it
...
"""
    r   = exec_python_remote(code)
    out = (r.get("result") or r.get("outputs") or {})
    if not out.get("state") and not r.get("success", True):
...
def main():
    ...
    step( 4, "bp_get_graph_summary_vars",     step4_wrapper)
    step( 5, "bp_get_graph_summary_fns",      step5_function_graphs)
    step( 6, "bp_get_graph_detail_tokens",    step6_graph_detail_tokens)
    ...
    ok15 = passed == 14  # 14 previous steps passed
    step_results.append({"step": 15, "name": "final_summary_assertion", "ok": ok15,
                          "ms": 0, "tokens": 0,
