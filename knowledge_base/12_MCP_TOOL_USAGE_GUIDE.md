...
### `import_sound_asset` ⭐ NEW (direct UE5 host path)

Imports a WAV, OGG, or MP3 file that **already exists on the Windows machine running UE5** (e.g. placed there manually, or downloaded via a browser on the UE5 machine) directly into the Content Browser as a SoundWave.

```bash
python3 sandbox_ue5cli.py exec_python '{
  "code": "..."  # not used directly — call via MCP tool
}'
# OR call via MCP:
import_sound_asset(
...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `file_path` | `str` | required | Absolute OS path on the UE5 Windows machine (e.g. `"C:/Sounds/jump.wav"`) |
| `destination_path` | `str` | `"/Game/Audio/"` | Content Browser folder for the imported SoundWave asset |
| `auto_create_cue` | `bool` | `False` | If `True`, also creates a SoundCue (`<name>_Cue`) wired to the SoundWave in the same folder |

**Returns (JSON string):**

Success:
```json
{"success": true, "asset_path": "/Game/Audio/jump", "asset_type": "SoundWave"}
```
Success with cue:
```json
{"success": true, "asset_path": "/Game/Audio/jump", "asset_type": "SoundWave", "cue_path": "/Game/Audio/jump_Cue"}
```
Failure:
```json
{"success": false, "error": "Import returned no paths. Check that the file exists on the UE5 host machine at: C:/Sounds/jump.wav"}
```
...
- The asset name is derived from the filename (no extension): `C:/Sounds/SFX_TurretFire.wav` → `SFX_TurretFire`.
- Returns the clean Content Browser path (no `.AssetName` suffix).
- For files **on the sandbox** (generated by `audio_generation` or downloaded with `DownloadFileWrapper`), use `import_sound_asset_from_sandbox` instead — it base64-transfers the file to the UE5 machine automatically.

---

### `import_sound_asset_from_sandbox` (sandbox → UE5 transfer)

Imports an audio file **from the sandbox Linux filesystem** into UE5. Base64-encodes the file, transfers it to the UE5 Windows machine's temp folder, then imports it with AssetTools.

```bash
python3 sandbox_ue5cli.py import_sound_asset_from_sandbox '{
  "local_file_path": "/home/user/webapp/SFX_TurretFire.mp3",
  "asset_name": "SFX_TurretFire",
...
**Typical workflow:**
1. `audio_generation(...)` → get Genspark file URL
2. `DownloadFileWrapper(url, destination_directory="/home/user/webapp/")` → saves to sandbox
3. `import_sound_asset_from_sandbox(local_file_path="/home/user/webapp/SFX_TurretFire.mp3", asset_name="SFX_TurretFire", ...)`

---

### `wire_play_sound_to_blueprint`

Add a `PlaySound2D` or `PlaySoundAtLocation` node to a Blueprint, wired after a specified node.

```bash
python3 sandbox_ue5cli.py wire_play_sound_to_blueprint '{
  "blueprint_name": "BP_LaserTurret",
  "sound_asset_path": "/Game/Audio/SFX_TurretFire.SFX_TurretFire",
  "after_node_id": "<node_guid>",
  "node_position": [400, 200],
  "use_play_at_location": false
}'
```
...
| `import_sound_asset` returns `{"success": false, "error": "Import returned no paths"}` | File path doesn't exist on UE5 Windows machine | Check path with `os.path.exists` inside an `exec_python` call; use `import_sound_asset_from_sandbox` if file is on sandbox |
