...
- Current tracked-worktree audit finds 0 Python commands without C++ routes and 0 C++ routes not referenced by Python.
- Residual aliases, internal helpers, legacy material/actor routes, and sample AI recipe routes are now acknowledged through opt-in compatibility dispatch on the richer public Python tools.
- Native-route parity wrappers are now live for `set_behavior_tree_blackboard`, `bt_get_info`, `add_niagara_component`, `set_blueprint_parent_class`, `set_pawn_properties`, `reconstruct_blueprint_node`, `set_spawn_actor_class`, `rename_blueprint_comment_node`, `add_blueprint_custom_event_node`, `add_blueprint_function_with_pins`, `add_interface_event_node`, `add_custom_event`, `call_custom_event`, `add_sequence_player_node`, `connect_anim_graph_nodes`, `add_clear_blackboard_value_node`, `add_finish_execute_node`, `add_get_random_reachable_point_node`, `add_pawn_sensing_component`, `create_bt_attack_task`, `create_bt_wander_task`, `create_enemy_spawner_blueprint`, `create_full_upgraded_enemy_ai`, and `attach_bt_sub_node`; existing graph/data/editor/material helpers route supported `add_open_level_node`, `add_arithmetic_operator_node`, `add_relational_operator_node`, `add_construction_script_node`, `add_map_variable`, `create_actor`, `add_blueprint_set_component_property`, and `set_material_instance_parameter` calls through their native command names.
- Offline tests now assert that the registry has no Python-missing-C++ commands and no C++-only routes when compatibility dispatch is present.
