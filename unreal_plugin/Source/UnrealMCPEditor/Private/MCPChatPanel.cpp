...
FString SMCPChatPanel::BuildPlayableSlicePrompt() const
{
	...
	return FString::Printf(
		...
		TEXT("Workflow:\n")
		TEXT("1. Call `skill_generate_playable_slice` with mode=\"preflight\" and session_name=\"%s\". Report any missing live-readiness gates before planning paid work; this mode makes no Tripo request, does not mutate Unreal, and does not reserve credits.\n")
		TEXT("2. Call `skill_generate_playable_slice` with mode=\"plan\", brief=\"%s\", content_path=\"%s\", asset_roles=\"%s\", gameplay_loop=\"%s\", acceptance_criteria=\"%s\", and required_evidence=\"%s\". Use the returned `unreal_mcp_playable_slice_plan.v1` as the source of truth for asset roles, paths, gameplay targets, and validation.\n")
		TEXT("3. Discover current state with `get_project_context`, `scan_project_assets(path=\"/Game\", depth=3)`, `list_available_tools(domain=\"all\")`, and `get_onboarding_context(task=\"blueprints\")`, `get_onboarding_context(task=\"world_building\")`, and `get_onboarding_context(task=\"umg\")`.\n")
		TEXT("4. Produce a short execution plan that maps each generated_asset_role to a concrete Unreal asset path under \"%s\" and lists the Blueprints, widgets, actors, or level assets to create or modify. Ask only before destructive overwrite.\n")
		...
		TEXT("6. Call `skill_generate_playable_slice` with mode=\"orchestrate\", the same asset_roles/gameplay_loop/acceptance_criteria/required_evidence, and any submitted task JSON to get the import/gameplay/PIE/report execution package. Inspect the returned `evidence_readiness`; it should stay `live_playable_slice_proven=false` until real import, compile, PIE, screenshot, and report artifacts exist. For each Tripo task, call `gen_tripo_wait_for_task` until complete, then call `gen_tripo_import_to_project` with content_path \"%s\" and a role-specific asset_name. Record task_id, credit usage, imported asset paths, materials, and warnings.\n")
		TEXT("7. If a hero asset needs extra art direction, use the Texture/Paint path: first call `gen_prepare_texture_paint_session`, then use `gen_tripo_magic_brush_generate`, `gen_tripo_magic_brush_get_retexture`, optional `gen_tripo_magic_brush_list_images`, and `gen_tripo_magic_brush_apply` only when project/render/image_map data and spend approval are available.\n")
		TEXT("8. Build the playable loop using the appropriate Blueprint, actor, component, UMG, AI, level, lighting, collision, and placement MCP tools. Prefer small searchable graph/function names, reusable Actor Components for repeated behavior, and visible player feedback.\n")
		TEXT("9. Run `compile_blueprint_and_report` for every touched Blueprint or Widget Blueprint, save changed assets, and use readback/diagnostic tools to confirm generated meshes, materials, collision, references, and gameplay wiring are actually assigned.\n")
		TEXT("10. Verify runtime with `pie_launch_session`, exercise the loop as much as tools allow, capture `pie_capture_log`, capture `viewport_capture_screenshot`, then run `pie_stop_session`.\n")
		TEXT("11. Finish with `skill_package_vertical_slice_report`, then build an `execution_evidence_json` object containing credit_guard or credit_usage, gameplay_assets, compile_reports, pie_log_path, pie_duration_s, viewport_screenshot_path, and vertical_slice_report_path. Call `skill_generate_playable_slice` again with mode=\"orchestrate\", task_submissions_json, imported_assets_json, and execution_evidence_json. Report `evidence_readiness.live_playable_slice_proven`, any incomplete evidence gates, changed asset paths, Tripo task ids, compile status, PIE/log/screenshot evidence paths, unresolved warnings/errors, and what remains for human design review."),
